...
#   PACKAGE_LIST_URL="https://md.archlinux.org/s/SxbqukK6IA/download" ./aur_check.sh
#
# Exit codes:
#   0 = clean
...
# ---------------------------------------------------------------------------
# Load package list from remote URL (single source of truth)
# Can be overridden via PACKAGE_LIST_URL env var
# ---------------------------------------------------------------------------
PACKAGE_LIST_URL="${PACKAGE_LIST_URL:-https://md.archlinux.org/s/SxbqukK6IA/download}"
...
fetch_url() {
    ...
    if command -v curl >/dev/null 2>&1; then
        ...
        echo >&2 "ERROR: Neither curl nor wget available to fetch package list."
        return 1
    fi
}

PACKAGE_LIST_DATA=$(fetch_url "$PACKAGE_LIST_URL") || {
    echo >&2 "ERROR: Failed to fetch package list: $PACKAGE_LIST_URL"
    echo >&2 "Check network or set PACKAGE_LIST_URL to a reachable source."
    exit 1
}
...
while IFS= read -r line; do
    ...
    INFECTED_PKGS+=("$line")
done <<< "$PACKAGE_LIST_DATA"

if [[ ${#INFECTED_PKGS[@]} -eq 0 ]]; then
    echo >&2 "ERROR: Package list fetched but empty: $PACKAGE_LIST_URL"
    exit 1
fi
